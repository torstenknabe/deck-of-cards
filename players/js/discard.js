var Discard = (function () {
  ...
  var flip = {
    discard: function discard(_discard) {
      _discard.flip = _discard.queued(flip);

      function flip(next, side) {
        var flipped = _discard.cards.filter(function (card) {
          return card.side === 'front';
        }).length / _discard.cards.length;

        _discard.cards.forEach(function (card, i) {
  ...
  var sort = {
    discard: function discard(_discard2) {
      _discard2.sort = _discard2.queued(sort);

      function sort(next, reverse) {
        var cards = _discard2.cards;
  ...
  // console.log("arrayvar" + _discard3);

  var shuffle = {

    discard: function discard(_discard3) {

      _discard3.shuffle = _discard3.queued(shuffle);
      function shuffle(next) {

        var cards = _discard3.cards;
  ...
  var poker = {
    discard: function discard(_discard4) {
      _discard4.poker = _discard4.queued(poker);

      function poker(next) {
        var cards = _discard4.cards;
  ...
  var intro = {
    discard: function discard(_discard5) {
      _discard5.intro = _discard5.queued(intro);

      function intro(next) {
        var cards = _discard5.cards;
  ...
  var fan = {
    discard: function discard(_discard6) {
      _discard6.fan = _discard6.queued(fan);

      function fan(next) {
        var cards = _discard6.cards;
        var len = cards.length;
  ...
  var bysuit = {
    discard: function discard(_discard7) {
      _discard7.bysuit = _discard7.queued(bysuit);

      function bysuit(next) {
        var cards = _discard7.cards;
  ...
  function Discard(jokers) {
    // init cards array
    var cards = new Array(jokers);
    ...
    function mount(root) {
      // mount discard to root
      $root = root;
      $root.appendChild($el);
    }

    function unmount() {
      // unmount discard from root
      $root.removeChild($el);
    }

    function addModule(module) {
      module.discard && module.discard(self);
    }
  }

  Discard.animationFrames = animationFrames;
  Discard.ease = ease;
  Discard.modules = { bysuit: bysuit, fan: fan, intro: intro, poker: poker, shuffle: shuffle, sort: sort, flip: flip };
  Discard.Card = _card;
  Discard.prefix = prefix;
